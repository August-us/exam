...
class Solution:
    def calculateMinimumHP(self, dungeon: List[List[int]]) -> int:
        dungeon[-1][-1] = min(-1, dungeon[-1][-1] -1)
        for j in range(len(dungeon[0])-1, 0, -1):
            dungeon[-1][j-1] = min(-1, dungeon[-1][j-1] + dungeon[-1][j])
        for i in range(len(dungeon)-1, 0, -1):
            dungeon[i-1][-1] = min(-1, dungeon[i-1][-1] + dungeon[i][-1])
            for j in range(len(dungeon[0]) - 1, 0, -1):
                dungeon[i-1][j - 1] = min(-1, dungeon[i-1][j - 1] + max(dungeon[i-1][j], dungeon[i][j-1]))
        print(dungeon)
        return -dungeon[0][0]
