    ...
    while nodes:
        ...
        for v in visited:
            for d in set(graph[v].keys()) - visited:
                ...
                if d == k and new_distance == graph[src].get(d, float('inf')):
                    pre.add(v)
                if new_distance < mid_distance:
                    mid_distance=new_distance

                    if new_distance < graph[src].get(d,float('inf')):
...
class Solution127:
    ...
    def ladderLength(self, beginWord: str, endWord: str, wordList: list) -> list:
        ...
        forward, backward, curf, curb = {beginWord: 1}, {endWord: 1}, {beginWord: 1}, {endWord: 1}
        f = forward.copy()
        while curf and curb:
            if len(forward) > len(backward):  # 始终从路径分支较少的一端做BFS
                ...
                curf, curb = curb, curf
            tmp = {}
            for word, paths in curf.items():  # 取出路径结束词以及到达它的所有路径
                for i in range(len(word)):
                    for a in 'abcdefghijklmnopqrstuvwxyz':
                        new = word[:i] + a + word[i + 1:]  # 对结束词尝试每一位的置换
                        if new in backward:  # 如果在后向分支列表里发现置换后的词，则路径会合并
                            return paths + backward[new]
                        if new in forward or new in tmp:
                            continue
                        if new in wordList:  # 仅当wordList存在该词且该词还未碰见过才进行BFS
                            tmp[new] = 1 + paths
            forward.update(tmp)  # 更新前向分支
            curf = tmp.copy()
        return 0


start = time()
print(Solution127().ladderLength(beginWord,endWord,wordList))
print(time() - start)
...
class Solution126:
    ...
    '''''
    def findLadders_origin(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        def distance(i, j):
            if len(wordList[i]) != len(wordList[j]):return 0
            res = 0
            for i,j in zip(wordList[i],wordList[j]):
                res += int(i!=j)
            return res
        lenw = len(wordList)
        endIndex = wordList.index(endWord)
        connect = [{ } for i in range(lenw)]
        for i in range(lenw):
        ...
        dis,path  = dijkstra(connect.copy(),beginIndex)
        if dis[endIndex] ==float('inf'): return []
        res = []
        def path(cur,pre,last):
            if last ==1:
                if endIndex in connect[cur]:
                    res.append([wordList[i] for i in pre+[endIndex]])
                return
            for i in connect[cur].keys():
                if i not in pre:
                    pre.append(i)
                    path(i,pre,last-1)
                    pre.pop()
        path(beginIndex,[beginIndex],dis[endIndex])
        return  res


    def findLadders_optiAdjacencyList(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        if endWord not in wordList: return []
        if beginWord not in wordList:
            wordList.append(beginWord)
            beginIndex = len(wordList) - 1
        ...
        from time import time
        start = time()
        # 快速求邻接表的方法，把单词中的每个字母用*码掉，然后建立字典，如果能码出同样的单词，则距离为1
        for i in range(lenw):
            for j in range(len(wordList[i])):
                tmp = wordList[i][:j]+'*'+wordList[i][j+1:]
                map[tmp].append(i)
        for _,v in map.items():
            if len(v)>1 :
                for i in v:
                    connect[i].update( {j:1 for j in v if i!=j} )

        print(time()-start)
        start = time()
        # 这里是时间瓶颈，还是双向bfs更快一些
        dis,pre = dijkstra(connect.copy(), beginIndex,endIndex)
        print(pre)
        ...
        for i in range(1,lenP):
            ...
            for p in pre:
                for node in path[i]:
                    if p[-1] in connect[node]:
                        cur.append(p+[node])
            pre = cur
        pre = [i+[endIndex] for i in pre if i[-1] in connect[endIndex]]
        print(time() - start)

        return [[wordList[i] for i in j] for j in pre]

    def findLadders_bfs(self, beginWord: str, endWord: str, wordList: list) -> list:
        wordList = set(wordList)  # 转换为hash实现O(1)的in判断
        if endWord not in wordList:
            return []
        # 分别为答案、用于剪枝的已访问哈希，前向分支和后向分支，当前的前向分支以及后向分支中的路径和的长度
        ...
        for _, v in map.items():
            if len(v) > 1:
                for i in range(len(v)):
                    connect[v[i]] += v[:i] + v[i+1:]

        res, visited, forward, backward, _len = [], set(), {beginIndex: [[beginIndex]]}, {endIndex: [[endIndex]]}, 2
        while forward:
            if len(forward) > len(backward):  # 始终从路径分支较少的一端做BFS
                forward, backward = backward, forward
            tmp = {}  # 存储新的前向分支
            while forward:
                word, paths = forward.popitem()  # 取出路径结束词以及到达它的所有路径
                visited.add(word)  # 记录已访问
                for new in connect[word]:
                    if new not in visited:  # 仅当wordList存在该词且该词还未碰见过才进行BFS
                        tmp[new] = tmp.get(new, []) + [path + [new] for path in paths]
                    if new in backward:  # 如果在后向分支列表里发现置换后的词，则路径会合并
                        if paths[0][0] == beginIndex:  # 前向分支是从beginWord开始的，添加路径会和的笛卡尔积
                            res.extend([[wordList[i] for i in fPath + bPath[::-1]] for fPath in paths for bPath in backward[new]])
                        else:  # 后向分支是从endWord开始的，添加路径会和的笛卡尔积
                            res.extend([[wordList[i] for i in (bPath + fPath[::-1])] for fPath in paths for bPath in backward[new]])
...
start = time()
print(Solution126().findLadders(beginWord,endWord,wordList))
print(time() - start)

start = time()
print(Solution126().findLadders_bfs(beginWord,endWord,wordList))
print(time() - start)
