...
class Solution1:
    ...
    def InversePairs1(self, data):
        return sum(map(lambda x:sum([data[x]<data[i] for i in range(0,x)]),range(1,len(data))))%1000000007
    ...
    def InversePairs(self, data):
        ...
        for i in range(1,len(data)):
            if data[i]<data[pivot]:
                ...
                pivot=self.partition(data,0,pivot)
            else:
                pivot=self.partition(data,pivot+1,i)
            count=(count+i-pivot)%1000000007
        return count
...
count = 0
import time,random
# num=random.sample(range(1,int(1e4)),int(5e3))

num1=num.copy()
num2=num.copy()
start=time.time()
print(Solution().InversePairs(num))
print(time.time()-start)
start=time.time()
print(Solution1().InversePairs1(num1))
print(time.time()-start)
start=time.time()
print(Solution3().InversePairs(num2))
print(time.time()-start)